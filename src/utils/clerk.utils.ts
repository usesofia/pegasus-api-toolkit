...
export const buildClerkClientMock = () => {
  ...
  const clerkMemberships: OrganizationMembership[] = [
    ...
    buildClerkOrganizationMembership({
      ...
      clerkOrganization:
        clerkOrganizations[TestOrganization.CHILLIBEANS_STORE_312],
      role: 'org:admin',
    }),
    buildClerkOrganizationMembership({
      clerkUser: clerkUsers[TestUser.DANIEL],
      clerkOrganization:
        clerkOrganizations[TestOrganization.NAGUMO_SUPERMERCADOS],
      role: 'org:admin',
    }),
    buildClerkOrganizationMembership({
      clerkUser: clerkUsers[TestUser.RONALDO],
    ...
    getAuthUserEntity: ({ token }: { token: string }): AuthUserEntity => {
      ...
      if (organization) {
        ...
        const clerkMembership = clerkMemberships.find(
          (membership) =>
            membership.organization.id === clerkOrganization.id &&
            membership.publicUserData?.userId === clerkUser.id,
        );

        if (!clerkMembership) {
          throw new Error(
            `Membership not found for ${clerkUser.id} in ${clerkOrganization.id}.`,
          );
        }
        ...
        if (
          clerkOrganization.publicMetadata?.type === 'LEAF' &&
          clerkOrganization.publicMetadata.parent
        ) {
          const parentClerkOrganizationId =
            clerkOrganization.publicMetadata.parent;
          parentClerkOrganization = Object.values(clerkOrganizations).find(
            (organization) => organization.id === parentClerkOrganizationId,
          );
        }
        ...
        if (clerkOrganization.publicMetadata?.type === 'GROUP') {
          const childrenClerkOrganizationIds = clerkOrganization.publicMetadata
            .children as string[];
          childrenClerkOrganizations = Object.values(clerkOrganizations).filter(
            (organization) =>
              childrenClerkOrganizationIds.includes(organization.id),
          );
        }
    ...
    organizations: {
      createOrganizationInvitation: jest
        ...
        .mockImplementation(
          ({
            ...
            if (
              clerkInvitesByOrganization[organizationId].find(
                (invite) => invite.emailAddress === emailAddress,
              )
            ) {
              throw new Error(
                `Already invited ${emailAddress} to ${organizationId}.`,
              );
            }
      ...
      updateOrganizationMembership: jest
        ...
        .mockImplementation(
          ({
            ...
            if (!membership) {
              throw new Error(
                `Membership not found for ${userId} in ${organizationId}.`,
              );
            }
      ...
      revokeOrganizationInvitation: jest.fn().mockImplementation(
        ({
          ...
          const invite = clerkInvitesByOrganization[organizationId].find(
            (invite) => invite.id === invitationId,
          );

          if (!invite) {
            throw new Error(
              `Invitation not found for ${invitationId} in ${organizationId}.`,
            );
          }

          if (invite.status !== 'pending') {
            throw new Error(
              `Invitation is not pending for ${invitationId} in ${organizationId}.`,
            );
          }
      ...
      getOrganization: jest
        .fn()
        .mockImplementation(
          ({ organizationId }: { organizationId: string }): Organization => {
            const organization = [
              ...newClerkOrganizations,
              ...Object.values(clerkOrganizations),
      ...
      updateOrganization: jest.fn().mockImplementation(
        (
          ...
          const isNewOrganization = !!newClerkOrganizations.find(
          ...
          if (isNewOrganization) {
            ...
            const testOrganizationKey = Object.keys(TestOrganization).find(
              (testOrganizationKey) =>
                clerkOrganizations[testOrganizationKey as TestOrganization]
                  .id === organizationId,
            );
          ...
          ({
            ...
            if (query) {
              filteredOrganizations = organizations.filter((organization) =>
                organization.slug?.toLowerCase().includes(query.toLowerCase()),
              );
            }
