...
export const buildAuthServiceMock = ({
  ...
  return {
    generateGcpServiceAccountToken: jest
      .fn()
      .mockResolvedValue(GCP_SERVICE_ACCOUNT_TOKEN_FOR_TESTS),
    ...
    getUser: ({
      ...
      const clerkUser = Object.values(clerkClientMock._clerkUsers).find(
        (user) => user.id === userId,
      );
      ...
      const testUserKey = Object.entries(clerkClientMock._clerkUsers).find(
      ...
      if (organizationId) {
        const clerkOrganization = Object.values(
          clerkClientMock._clerkOrganizations,
        ...
        const testOrganizationKey = Object.entries(
          clerkClientMock._clerkOrganizations,
        ).find(([, value]) => value.id === organizationId)?.[0];
        ...
        return Promise.resolve(
          clerkClientMock.getAuthUserEntity({
            token: `${testUserKey}:${testOrganizationKey}`,
          }),
        );
      } else {
        return Promise.resolve(
          clerkClientMock.getAuthUserEntity({ token: testUserKey }),
        );
      }
    },
  };
